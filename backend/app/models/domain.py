...
class Task(Base):
    ...
    # 复杂结构采用 JSON 字符串化存储
    history = Column(Text, nullable=True, doc="JSON：按时间记录每节点输入/输出摘要、审查结论、错误信息")
    final_result = Column(Text, nullable=True, doc="最终交付 Markdown")
    token_usage = Column(Text, nullable=True, doc="JSON：按轮次/节点聚合的输入/输出 token")
    error_code = Column(String, nullable=True, doc="失败/异常分类（如 unrecognizable/timeout/provider_error 等）")
    
    # 审计与时间戳
    created_at = Column(DateTime(timezone=True), server_default=func.now(), doc="创建时间")
